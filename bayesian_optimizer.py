...
PROJECT_ID = "abis-345004" # Original from notebook, please replace with your actual project ID
LOCATION = "us-central1"
vertexai.init(project=PROJECT_ID, location=LOCATION)

# Safety settings (per the notebook)
SAFETY_SETTINGS = {
...
# Load the source text (hardcoded for now based on notebook/prompt.txt content)
# In a real app, this would be loaded from a file
BLOG_CONTENT = """In food and dining, customer reviews hold valuable insights. However, extracting meaning from large volumes of text and media data is challenging. Our team tackled this problem by developing Sentiment Chef, a smart restaurant agent that goes beyond simple review analysis. Here’s how we did it.
...
def generate_dataset(temperature, top_p):
    """Generates the Q&A dataset using Gemini 1.5 Flash."""
...
def evaluate_quality(generated_text):
    """Evaluates the quality of the generated dataset using Gemini 1.5 Pro as a judge."""
    if not generated_text:
        return 0.0

    judge_model = GenerativeModel("gemini-1.5-pro-001")
    
    judge_prompt = f"""
    You are an expert data quality evaluator for LLM fine-tuning datasets.
    
    Your task is to evaluate the quality of the following generated JSONL content.
    The content should be a list of JSON objects, each containing a 'messages' list with 'user' and 'model' roles.
    The 'model' response must be factually consistent with the provided source blog (which is implicitly the source of truth).
    
    Evaluate on the following criteria:
    1. **Format validity**: Is it valid JSONL? (Each line is a valid JSON object).
    2. **Relevance**: do the Q&A pairs make sense and are they derived from the blog context?
    3. **Diversity**: Are the questions distinct and cover different parts of the blog?
    4. **Hallucination**: Does the model invent facts not in the source? (Penalty for this).

    Return a single float score between 0.0 and 10.0, where 10.0 is perfect.
    
    Only return the numeric score. Do not return any other text.
    
    Generated Content to Evaluate:
    ```
    {generated_text[:15000]} # Truncate to avoid context limit if necessary, though 1.5 Pro has huge context
    ```
    """
    ...
    try:
        ...
        # Extract float from string in case of extra text
...
def objective(trial):
    ...
    print(f"Trial {trial.number}: Testing temp={temperature:.2f}, top_p={top_p:.2f}")
    
    generated_text = generate_dataset(temperature, top_p)
    
    # Quick sanity check for empty output
    if not generated_text:
        return 0.0
        
    score = evaluate_quality(generated_text)
    print(f"Trial {trial.number} Score: {score}")
    return score

if __name__ == "__main__":
    print("Starting Bayesian Optimization for Reverse RAG Parameters...")
    ...
    print(f"Best value (Quality Score): {study.best_value}")
    print("Best params:")
    for key, value in study.best_params.items():
        print(f"    {key}: {value}")
    
    # Optionally generate the final best dataset
    print("\nGenerating final dataset with best parameters...")
    best_temp = study.best_params["temperature"]
    best_top_p = study.best_params["top_p"]
    final_dataset = generate_dataset(best_temp, best_top_p)
    
    with open("best_dataset.jsonl", "w") as f:
        f.write(final_dataset)
    print("Best dataset saved to 'best_dataset.jsonl'.")
